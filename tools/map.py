...
ВАЖНО: пересобрал карту — подними версию в адресе картинки.
В сборках она вписана как `/img/map.webp?v=3` (и так для всех четырёх
файлов). Картинки отдаются с `max-age` в год, поэтому без смены адреса
у всех, кто уже был на сайте, ещё год будет висеть старая карта —
на это уже наступили один раз.
...
# Палитра сайта, тёмный вариант — в цвет navy-блоков страницы. Светлую
# карту пробовали и отказались: тёмно-синие магистрали на почти белом поле
# режут глаз сильнее, чем помогают. Здесь наоборот — фон глухой, а дороги
# светлая сталь, и кадр читается спокойно.
#
# Дома и мелкие площадки уводим в цвет фона: на карте такого размера
# застройка читается как рябь, а не как информация — человек смотрит сюда,
# чтобы понять «где это примерно», маршрут он всё равно строит у себя.
PAL = dict(
    LAND="0F212C",        # фон, чуть светлее --navy
    BUILD="0F212C",       # дома убраны
    BUILD_EDGE="0F212C",
    GREEN="122630",       # парки — едва светлее фона
    PITCH="16303E",       # поля стадионов
    WATER="E0700C",       # --accent, Салар
    RAILAREA="132734",    # полоса отвода железной дороги
    RAIL="93A9B8",        # рельсы
    MAJOR="7B95A8",       # магистрали — светлая сталь
    PRIMARY="5E7A8D",     # главные улицы
    SECOND="1E3441",      # второстепенные — почти фон
    ROAD="3A5465",        # мелкие улицы
    ROAD_EDGE="172C37",   # кромка улиц
    LABEL="0F212C",       # подписи OSM стираем, вместо них свои
    POI="0F212C",         # значки заведений тоже
    HALO="0F212C",        # ореол подписей — в цвет подложки
    MARK="DCE6EC",        # обод метки офиса, светлый на тёмном
    MARK_DOT="E0700C",    # сердцевина метки
)

INK, MUTED = rgb("E8EFF3"), rgb("B9CBD6")
...
LABELS = [
    ...
    (293, 340, "Салар",            600, 15, 0.10, rgb("E8913C"),  0),
...
def text_layer(text, font, fill, track_em):
    """Слово с разрядкой и ореолом на прозрачном слое.

    Ореол нужен всегда: подпись ложится и на фон, и на полосу дороги,
    и без него на дороге она пропадает. Цвет ореола — цвет подложки
    карты: на светлой он белый, на тёмной должен быть тёмным, иначе
    подпись обрастает светящейся каймой.
    """
    halo_rgb = rgb(PAL["HALO"])
    pad = 6
